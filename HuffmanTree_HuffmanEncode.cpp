...
int main() {
    ...
    root = buildHuffman(pq);
    Traverse(root,toDic);
    cout << endl << "编码结果:" << endl;
    int len = text.length();
    for(int i = 0; i < len; i++) {
        ans = ans + toDic[text[i]];
    }
    cout << ans << endl;
    // cout << "bin_code:"<< endl;
    // cin >> bin_code;
...
void Traverse(TreeNode<char> *root, unordered_map<char, string> &_toDic) {
    if(root->isLeaf()) {
        root->print();
        _toDic[root->element] = root->coding;
        return;
    }
...
bool saveDic(unordered_map<char,string> &dic, string filename = ".\\huffmandic.csv") {
    ...
    for(auto &x: dic) {
        fout << x.first << ',' << x.second.c_str() << endl; 
    }
    fout.close();
    return true;
}